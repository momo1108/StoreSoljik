    ...
    const loadPostcodeScript = async () => {
      try {
        await loadScript(
          ...
          {
            message: 'Daum 우편번호 기능을 불러왔습니다.',
            description: '"우편번호 찾기" 버튼을 통해 주소를 검색하세요.',
          },
          {
            message: 'Daum 우편번호 기능을 불러오는데 실패했습니다.',
            description:
              '직접 입력하시거나 "새로고침"을 통해 다시 페이지를 로딩해주세요.',
          },
        );
        setScriptLoaded(true);
      } catch (error) {
        toast.error('Daum 우편번호 기능을 불러오는데 실패했습니다.');
      }
    };
